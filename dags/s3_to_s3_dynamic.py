...
def s3_to_s3_dynamic():
    def _filter_keys(
        keys: list, from_datetime: datetime, to_datetime: datetime
    ) -> list:
        """
        ...
        filtered_keys = [
            k["Key"] for k in keys if Path(k["Key"]).suffix in ending_formats
        ]
    ...
    def copy_object(key: str) -> None:
        ...
        dest_bucket, dest_prefix = (
            dagrun_conf["dest_bucket"],
            dagrun_conf["dest_prefix"],
        )
